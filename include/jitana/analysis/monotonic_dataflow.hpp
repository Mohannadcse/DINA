...
namespace jitana {
    template <typename Graph, typename SetMap, typename CombOp,
              typename FlowFunc>
    void monotonic_dataflow(const Graph& g, SetMap& inset_map,
                            SetMap& outset_map, CombOp comb_op,
                            FlowFunc flow_func)
    {
        ...
        struct rev_postorder_maker : public boost::default_dfs_visitor {
            std::queue<vertex_descriptor>& worklist;
            using reverse_graph = decltype(rg);
            ...
            void finish_vertex(const vertex_descriptor& u, const reverse_graph&)
            {
                worklist.push(u);
            }
        } vis(worklist);
