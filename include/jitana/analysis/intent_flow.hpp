    ...
    namespace detail {
        inline auto compute_explicit_intent_handlers(virtual_machine& vm)
        {
            ...
            for (const auto& lv : boost::make_iterator_range(vertices(lg))) {
                std::cout << "loaderName:: " << lg[lv].loader.name() << "\n";
        ...
        inline auto compute_implicit_intent_handlers(virtual_machine& vm)
        {
            ...
            for (const auto& lv : boost::make_iterator_range(vertices(lg))) {
                ...
                BOOST_FOREACH(auto &v, info->manifest_ptree().get_child("application")) {
                    auto subtree = v.second;
                    if (v.first == "activity" || v.first == "receiver" || v.first == "service"){
                        BOOST_FOREACH( auto const& v2, subtree.get_child( "" ) ){
                            if (v2.first == "intent-filter"){
                                auto subtree2 = v2.second;
                                BOOST_FOREACH( auto const& v3, subtree2.get_child( "" ) ){
                                    if (v3.first == "action"){
                                        std::cout << "v3.second:: " << v3.second.get_optional <std::string> ("<xmlattr>.android:name") << "\n";
                                        auto name = v3.second.get<std::string>("<xmlattr>.android:name");
